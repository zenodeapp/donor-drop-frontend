const randomTransparentAddresses = [
...
const getRandomTnam = () => {
  const randomIndex = Math.floor(
    Math.random() * randomTransparentAddresses.length
  );
  return randomTransparentAddresses[randomIndex];
};

const generateRandomValue = (minEth: number, maxEth: number) =>
  (Math.random() * (maxEth - minEth) + minEth).toFixed(18);

const generateRandomBlock = () => {
  return BigInt(Math.ceil(Math.random() * Number(99999999n)));
};
